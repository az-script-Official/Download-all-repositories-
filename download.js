...
/**
 * fetch مع timeout لمنع التعليق اللانهائي
 */
...
/**
 * تحميل ملف ZIP لمستودع واحد عبر الـ Worker مع retry و timeout
 */
async function fetchRepoZip(zipUrl, retries = 3) {
    const workerUrl = `${WORKER_URL}?url=${encodeURIComponent(zipUrl)}`;

    for (let attempt = 1; attempt <= retries; attempt++) {
        try {
            const resp = await fetchWithTimeout(workerUrl, {}, 30000);

            if (resp.status === 429) {
                // Rate limited - انتظار أطول مع كل محاولة
                const waitTime = Math.min(5000 * attempt, 30000);
                console.warn(`Rate limited, انتظار ${waitTime}ms... (محاولة ${attempt}/${retries})`);
            ...
            if (err.name === 'AbortError') {
                console.warn(`Timeout في تحميل ${zipUrl} (محاولة ${attempt}/${retries})`);
            } else {
                console.warn(`خطأ في تحميل ${zipUrl}: ${err.message} (محاولة ${attempt}/${retries})`);
            }

            if (attempt === retries) {
                throw new Error(`فشل بعد ${retries} محاولات: ${err.message}`);
            }
    ...
    throw new Error(`فشل تحميل: ${zipUrl}`);
}
...
async function downloadReposForAccount(repos, onProgress) {
    ...
    // تقليل التوازي إلى 3 لتجنب rate limiting
    const CONCURRENT = 3;
    ...
    // معالجة الدفعات مع تأخير بين كل دفعة
    for (let i = 0; i < total; i += CONCURRENT) {
        ...
        // تأخير بين الدفعات لتجنب rate limiting (بعد كل دفعة ما عدا الأخيرة)
...
async function startSequentialDownload(accounts, callbacks) {
    ...
    for (let i = 0; i < accounts.length; i++) {
        ...
        try {
            ...
            // انتظار قبل الحساب التالي لتجنب rate limiting
